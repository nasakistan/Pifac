...
| `ScrapingMethod` | Scraping method | Property | [`NScrapingMethod`](common/NScrapingMethod.md) |  | `NScrapingMethod.Default` |  | The scraping method used to get the text. |
...
|------|-------------|------|-------------|
| `TextString` | Text | `OutArgument<string>` | Where to save the copied text. |
...
```bash
uip rpa get-default-activity-xaml --activity-class-name UiPath.UIAutomationNext.Activities.NGetText
```
## Notes
